...
export class DependentsComponent implements OnInit {
  ...
  trackId(index: number, item: IDependents): number {
    return item.id!;
  }

  delete(dependents: IDependents): void {
    const modalRef = this.modalService.open(DependentsDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.dependents = dependents;
  ...
  protected onSuccess(data: IDependents[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
